...
# Diálogo (nuevo)
@export var dialogue_path: String = "res://Dialogos/Arbol_caido.dialogue"
...
const DIALOGUE_NODE := "arbol_caido"
...
# ==================================================
func _ready() -> void:
	# Si ya lo usaste en este slot, el árbol se queda, pero no vuelve a hablar
	used = GameData.hoja_encontrada_done

	area.body_entered.connect(_on_area_entered)
	area.body_exited.connect(_on_area_exited)

	# Retícula / puntero (tu CenterContainer con Reticula.gd)
...
func _interactuar() -> void:
	...
	# Apaga el área para evitar spam (pero el árbol y su colisión se quedan)
	area.monitoring = false
	area.monitorable = false
...
func _on_dialogo_terminado() -> void:
	# Guardar flag (reutilizo tu flag existente)
